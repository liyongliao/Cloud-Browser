...
function validateStep() {
  const controls = [
    ...fields[step].querySelectorAll("input:not(.hidden input),select"),
  ];
...
function renderSummary() {
  const mode =
    form.databaseMode.value === "internal"
      ? "内置 PostgreSQL"
      : `${form.databaseHost.value}:${form.databasePort.value}`;
  const gateway =
    form.gatewayMode.value === "direct" ? "自动 HTTPS" : "已有反向代理";
  document.querySelector("#summary").innerHTML =
    `<div><span>访问地址</span><strong>https://${escapeHTML(form.domain.value)}</strong></div><div><span>接入方式</span><strong>${gateway}</strong></div><div><span>数据库</span><strong>${escapeHTML(mode)}</strong></div><div><span>管理员</span><strong>${escapeHTML(form.adminEmail.value)}</strong></div><div><span>并发会话</span><strong>${form.maxSessions.value} 个</strong></div>`;
}
...
document.querySelector("#generate").onclick = () => {
  ...
  form.databasePassword.value = [...bytes]
    .map(
      (byte) =>
...
form.databaseMode.forEach(
  (control) =>
    (control.onchange = () => {
      const external = form.databaseMode.value === "external";
      document.querySelector("#external").classList.toggle("hidden", !external);
      document.querySelector("#ssl").classList.toggle("hidden", !external);
    }),
);
form.addEventListener("submit", async (event) => {
  ...
  payload.databasePort = Number(payload.databasePort || 5432);
  payload.maxSessions = Number(payload.maxSessions);
...
function failed(message) {
  document.querySelector("#install-title").textContent = "安装没有完成";
